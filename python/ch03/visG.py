...
def visG(T, cname):
    ...
    """
    cv = np.array([
        [22.0900, 76.9000, -211.6000], [5.1750, 45.6900, -88.5400],
    ...
    wv = np.column_stack([cv[:, 0], cv[:, 1] * 1e-2, cv[:, 2] * 1e-6])
    ind = compID(cname) - 1 # MATLAB 1-based indexing 보정
    
    T_arr = np.array(T)
    pn = wv[ind, 0] + wv[ind, 1] * T_arr + wv[ind, 2] * (T_arr**2)
    mu = pn * 1e-4
